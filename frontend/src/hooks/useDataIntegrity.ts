...
export const useDataIntegrity = (): UseDataIntegrityReturn => {
  ...
  const checkHealthStatus = useCallback(async (): Promise<void> => {
    try {
      setError(null);
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:3007'}/api/v1/integrity/health`);
  ...
  const checkIntegrity = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:3007'}/api/v1/integrity/check/complete`, {
        method: 'POST',
        headers: {
...
export const useRecordCountVerification = (expectedCount: number) => {
  ...
  const verifyCount = useCallback(async () => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:3007'}/api/v1/integrity/check/total-records`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        }
